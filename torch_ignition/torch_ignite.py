...
def create_logging_folder(log_dir: str, appname: str = "train"):
...
class TorchIgnite(object):
    ...
    def run(self, epochs: int = 1):
        ...
        def log_training_results(engine):
            ...
            self.writer.add_scalar(
                "training/avg_accuracy", avg_accuracy, engine.state.epoch
            )
        ...
        def log_validation_results(engine):
            ...
            self.writer.add_scalar(
                "valdation/avg_accuracy", avg_accuracy, engine.state.epoch
            )

        objects_to_checkpoint = dict(model=self.model, optimizer=self.optimizer)
